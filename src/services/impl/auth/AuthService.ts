...
export class AuthService implements IAuthService{
  ...
  async login(email: string, password: string): Promise<LoginResponse>{
    ...
    if (!user) {
      await bcrypt.compare(password, '$2b$12$dummyhashthatwontmatchanything123456');
      throw new AppError(
        'INVALID_CREDENTIALS',
  ...
  private async _generateTokens(payload: TokenPayload): Promise<TokenPair>{
   return Promise.resolve({accessToken: this.tokenService.generateAccessToken(payload),
