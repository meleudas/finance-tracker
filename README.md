...
npm install
docker compose up -d
# лише Postgres. Повний стек (Postgres + API): docker compose --profile full up -d
npx prisma migrate dev
npx prisma db seed
npm run dev
```

Сервер за замовчуванням: `http://localhost:3000`. Документація API (Swagger): `/api/docs` (після реалізації).
